...
public class TVec2LObjectHashMap<V> extends TVec2LHash {
...
//	};
	 ...
	 */
	public TVec2LObjectHashMap( int initialCapacity ) {
	 ...
	 */
	public TVec2LObjectHashMap( int initialCapacity, float loadFactor ) {
	 ...
	 */
	public TVec2LObjectHashMap( int initialCapacity, float loadFactor, long noEntryKey ) {
		super( initialCapacity, loadFactor );
	...
	protected void rehash( int newCapacity ) {
		int oldCapacity = _set_xy.length / 2;

		long oldKeys[] = _set_xy;
		V oldVals[] = _values;
		byte oldStates[] = _states;

		_set_xy = new long[newCapacity * 2];
		...
		for ( int i = oldCapacity; i-- > 0; ) {
			if( oldStates[i] == FULL ) {
				int i2 = i * 2;
				long o1 = oldKeys[i2];
				long o2 = oldKeys[i2+1];
				int index = insertKey(o1,o2);
	...
	/** {@inheritDoc} */
	public boolean containsKey( long keyX, long keyY ) {
		return contains( keyX, keyY );
	}
	...
	/** {@inheritDoc} */
	public V get( long keyX, long keyY ) {
		int index = index( keyX, keyY );
	...
	/** {@inheritDoc} */
	public V put( long keyX, long keyY, V value ) {
		int index = insertKey( keyX, keyY );
		return doPut( value, index );
	}


	/** {@inheritDoc} */
	public V putIfAbsent( long keyX, long keyY, V value ) {
		int index = insertKey( keyX, keyY );
		if ( index < 0 )
	...
	/** {@inheritDoc} */
	public V remove( long keyX, long keyY ) {
		V prev = null;
		int index = index( keyX, keyY );
		if ( index >= 0 ) {
			prev = _values[index];
	...
	*/
	...
	public void clear() {
		super.clear();
		Arrays.fill( _set_xy, 0, _set_xy.length, no_entry_key );
	...
	public TLongObjectIterator<V> iterator() {
		return new TLongObjectHashMap.TLongObjectHashIterator<V>( this );
	}*/
...
//	protected class ValueView extends TLongObjectHashMap.MapBackedView<V> {
//
//		@SuppressWarnings({"unchecked"})
